...
[Saiba como obter até 50% de desconto →](/landing)
    `,
  },
  {
...
function renderMarkdown(md: string): React.ReactNode[] {
  ...
  const inline = (text: string): React.ReactNode => {
    ...
    return parts.map((part, i) => {
      ...
      if (linkMatch) {
        return (
          <Link key={i} href={linkMatch[2]} className="text-sky-400 underline hover:text-sky-200">
            {linkMatch[1]}
          </Link>
        );
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line.trim()) {
      ...
      nodes.push(
        <h3 key={key++} className="text-sky-200 font-black text-xl mt-6 mb-2">
          {line.slice(4)}
        </h3>,
      );
    } else if (line.startsWith('## ')) {
      nodes.push(
        <h2 key={key++} className="text-sky-100 font-black text-2xl mt-8 mb-3"
          style={{
      ...
      nodes.push(
        <li key={key++} className="text-sky-300/80 text-base leading-relaxed ml-4 list-disc">
          {inline(line.slice(2))}
        </li>,
      );
    } else {
      nodes.push(
        <p key={key++} className="text-sky-300/80 text-base leading-relaxed">
          {inline(line)}
        </p>,
      );
    }
